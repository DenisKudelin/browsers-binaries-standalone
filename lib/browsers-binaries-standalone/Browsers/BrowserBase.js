...
var BrowserBase = (function () {
    function BrowserBase(name, platform, version, path) {
        this.name = name;
        this.version = version;
        this.platform = platform;
        this.path = path || externals_1.Path.join(__dirname, "../../../browsers/");
        if (!platform) {
    ...
    BrowserBase.prototype.getBinariesPath = function (path) {
        if (path === void 0) { path = ""; }
        var versionName = this.version.toLowerCase() + "_" + Browsers_1.Platform[this.platform].toLowerCase() + (this.versionSuffix || "");
        return externals_1.Path.join(this.path, this.name.toLowerCase(), versionName, path);
    };
    ...
    BrowserBase.prototype.downloadFile = function (url, unpack) {
        ...
        return externals_1.Promise.resolve()
            ...
            var progress = new exports_1.ProgressLog();
            request.pipe(fileStream);
            request.on("response", function (data) {
                var totalBytes = parseInt(data.headers["content-length"]);
